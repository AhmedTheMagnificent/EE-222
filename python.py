...
def landmark_to_servo_angle(hand_landmarks):
    ...
    print(angle, WRIST.x, INDEX_FINGER_MCP.x)
...
def map_angle_to_bytes(angle, angle_min, angle_max, out_min, out_max):
    mapped_angle = int((angle - angle_min) * (out_max - out_min) / (angle_max - angle_min) + out_min)
    mapped_angle = max(min(mapped_angle, out_max), out_min)  
    return mapped_angle.to_bytes(1, 'little')
        ...
        if results.multi_hand_landmarks:
            if len(results.multi_hand_landmarks) == 1:
                ...
                if servo_angle != prev_servo_angle:
                    ...
                    ser.write(bytearray(map_angle_to_bytes(servo_angle[0], x_min, x_max, 0, 255)))
                    ser.write(bytearray(map_angle_to_bytes(servo_angle[1], y_min, y_max, 0, 255)))
                    ser.write(bytearray(map_angle_to_bytes(servo_angle[2], z_min, z_max, 0, 255)))
                    ser.write(bytearray(map_angle_to_bytes(servo_angle[3], claw_close_angle, claw_open_angle, 0, 255)))
            else:
